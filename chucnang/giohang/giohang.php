    ...
    <table id="cart" class="table table-hover table-condensed">
        ...
        <?php  
            ...
            while ($row=mysqli_fetch_array($query)) {
                // Lấy giá trị từ cột 'gia_sp' trong hàng kết quả
            $gia_sp = $row['gia_sp'];
            // Loại bỏ dấu chấm từ giá trị và chuyển đổi thành số nguyên
            $gia_sp = intval(str_replace('.', '', $gia_sp));
                $totalPrice=$gia_sp*$_SESSION['giohang'][$row['id_sp']];
        ?>
        <tbody>
            <tr>
                <td data-th="Product">
